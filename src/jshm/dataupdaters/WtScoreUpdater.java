 ...
 *
 */
public class WtScoreUpdater {
	static final Logger LOG = Logger.getLogger(WtScoreUpdater.class.getName());
	
	public static void update(final boolean scrapeAll, final WtGame game, final Instrument.Group group, final Difficulty difficulty) throws Exception {
		update(null, scrapeAll, game, group, difficulty);
	}
	
	@SuppressWarnings("unchecked")
	public static void update(final ResultProgressHandle progress, final boolean scrapeAll, final WtGame game, final Instrument.Group group, final Difficulty difficulty) throws Exception {
		List<WtScore> scrapedScores =
			scrapeAll
			? jshm.sh.scraper.WtScoreScraper.scrapeAll(progress, game, group, difficulty)
			: jshm.sh.scraper.WtScoreScraper.scrapeLatest(progress, game, group, difficulty, null);
	...
	//			    		throw new IllegalStateException("found more than 1 existing score, found " + result.size());
			...
			LOG.throwing("WtScoreUpdater", "update", e);
			throw e;
		} finally {
